...
public class IL2Jasmin {
	...
	public void emmit(Klass m) {
		...
		{				
			...
			if( m.getLocal("main", PrimitiveType.INT) != null) 
			{
				/* produce a main stub */
				out.print("; Main stub\n");
		...
		for(Callable f : m.allCallables()) {
			if(f.declSignature().name.equals("<init>"))
				hasConstructor = true;
			this.emmit(f);
		}	
		
		if(!m.isInterface() && !hasConstructor) {
			/* TODO In general this is wrong, 'cause super type 
			 * doesn't have to have such constructor */
			out.print("; Default empty constructor\n");
		...
		if(op instanceof Call) {
			...
			if(c.target() instanceof Builtin) {					
				...
				switch(c.protocol()) {
					...
					case VIRTUAL:
						out.printf("%s %s/%s %s\n",
						(!c.target().container().isInterface() ? "invokevirtual" : "invokeinterface"),
						c.target().container().getKlassName().replace('.', '/'),
						JType.methodSignature(c.target().declSignature()), cnt );
						break;
					case NONVIRTUAL:
						out.printf("%s %s/%s %s\n",
							(!c.target().container().isInterface() ? "invokenonvirtual" : "invokeinterface"),
							c.target().container().getKlassName().replace('.', '/'),
							JType.methodSignature(c.target().declSignature()), cnt );
						break;						
				}
	...
	private static int calculateMaxStack(Operation op) {
		...
		throw new CompilerException(
			"Unmatched Operation in calcMaxStack: " + op.getClass().getCanonicalName());		
	}
}
